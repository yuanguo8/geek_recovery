...
int control_usb_storage_set_lun(Volume* vol, bool enable, const char *lun_file) {
    ...
    if ((write(fd, vol_device, strlen(vol_device) + 1) < 0) &&
       (!enable || !vol->blk_device || (write(fd, vol->blk_device, strlen(vol->blk_device)) < 0))) {
...
void cancel_mount_usb_storage_contral()
{
...
char* mg_choose_file_menu(const char* basedir,char *fileExtensionOrDirectory,Button_Box *father)
{
    Button_Box *files_button_box = calloc(sizeof(Button_Box),1);
    current_directory->button_box = files_button_box;
    files_button_box->x = 0;
    files_button_box->y = gr_fb_height() * 22/100;
    files_button_box->dx = gr_fb_width();
    files_button_box->dy = gr_fb_height() *78/100;
    files_button_box->back_button =gBack_to_previous_button;
    files_button_box->mask_button =gMask_button;
    files_button_box->dividing_line =gDividing_line;
    ...
    char** dirs = NULL;
    if (fileExtensionOrDirectory != NULL) dirs = gather_files(directory, NULL, &numDirs);

    int total = numDirs + numFiles;
    files_button_box->files_number = total;
    
    files_button_box->show_start_num =1;
    files_button_box->button_nun = ((files_button_box->dy)/(gr_get_height(files_button_box->back_button)))-1;

    if (files_button_box->button_nun > total)
    {
        files_button_box->button_nun = total;
    }
    files_button_box->show_end_num =files_button_box->show_start_num + files_button_box->button_nun-1;
    files_button_box->button_selected =0;
    ...
    files_button_box->text=list;
    mg_ui_update_file_manage(files_button_box);

    for (;;)
    {
        static char ret[PATH_MAX];
        int chosen_item = mg_get_menu_selection();
        if (chosen_item == GO_BACK || chosen_item == REFRESH || chosen_item == 0){
            current_directory->button_box=father;
            if (NULL!=father)
            {
                mg_ui_update_file_manage(father);
                free(files_button_box);
            }
            break;
        }
        else if ((chosen_item > 0) && (chosen_item <= numFiles))
        {
            strcpy(ret, files[chosen_item-1]);
            return_value = ret;
            mg_clear_button_box_select(files_button_box);
            free(files_button_box);
            break;
        }
        else if ((chosen_item >numFiles) && (chosen_item <= total))
        {
            char* subret = mg_choose_file_menu(dirs[chosen_item - numFiles -1], fileExtensionOrDirectory,files_button_box);
        if (subret != NULL)
        {
            strcpy(ret, subret);
            return_value = ret;
...
void mg_show_choose_zip_menu(char *mount_point)
{
    printf("%s\n",mount_point);
    ...
    char *file=mg_choose_file_menu(mount_point,".zip",NULL);
    if (file==NULL)
    {
...
int mg_show_partition_menu()
{
    ...
    while(1)
    {
        ...
        for (i = 0; list[i]!='\0'; i++);
        files_button_box->files_number=i;
        files_button_box->show_start_num =1;

        files_button_box->button_nun = ((files_button_box->dy)/(gr_get_height(files_button_box->back_button)))-1;
        if (files_button_box->button_nun > files_button_box->files_number)
        {
            files_button_box->button_nun = files_button_box->files_number;
        }
        files_button_box->show_end_num =files_button_box->show_start_num + files_button_box->button_nun-1;
        files_button_box->text=list;
        mg_ui_update_file_manage(files_button_box);
        chosen_item = mg_get_menu_selection();
        if (chosen_item == GO_BACK || chosen_item == REFRESH ||0 == chosen_item){
            mg_clear_button_box_select(files_button_box);
            break;
        }
        ...
        if (chosen_item == (mountable_volumes+formatable_volumes+1)) {
            ...
            show_mount_usb_storage_menu();
             ...
             MountMenuEntry* e = &mount_menu[chosen_item-1];
        ...
        else if (chosen_item < (mountable_volumes + formatable_volumes))
        {
            ...
            FormatMenuEntry* e = &format_menu[chosen_item-1];
...
int mg_backup_system_to_sdcard()
{
    ...
    if (tmp == NULL)
    {
        ...
        sprintf(backup_path, "%s/.mgyun/backup/%ld", chosen_path, tp.tv_sec);
    }
    else
    {
        char path_fmt[PATH_MAX];
        strftime(path_fmt, sizeof(path_fmt), ".mgyun/backup/%F.%H.%M.%S", tmp);
...
int  mg_restore_system_to_sdcard(char *path)
{
    nandroid_restore(path, 1, 1, 1, 1, 1, 0);
    return 0;
}
