...
// Should only be called with gUpdateMutex locked.
void mg_draw_tiptext(gr_surface *tiptex)
{
    if(tiptex) {
        int iconWidth = gr_get_width(tiptex);
        int iconHeight = gr_get_height(tiptex);
        int iconX = (gr_fb_width() - iconWidth) / 2;
        int iconY = gr_fb_height() *11/100;
        gr_blit(tiptex, 0, 0, iconWidth, iconHeight, iconX, iconY);
        gr_flip();
    }
}
...
void ui_init(void)
{
    ...
    for (i = 0; BITMAPS[i].name != NULL; ++i) {
        int result = res_create_surface("/res/images",BITMAPS[i].name, BITMAPS[i].surface);
        if (result < 0) {
    ...
    for (i = 0; i < ui_parameters.indeterminate_frames; ++i) {
        ...
        int result = res_create_surface("/res/images",filename, gProgressBarIndeterminate+i);
        if (result < 0) {
            LOGE("Missing bitmap %s\n(Code %d)\n", filename, result);
        }
    }
    ...
    if (ui_parameters.installing_frames > 0) {
        ...
        for (i = 0; i < ui_parameters.installing_frames; ++i) {
            ...
            int result = res_create_surface("/res/images",filename, gInstallationOverlay+i);
            if (result < 0) {
                LOGE("Missing bitmap %s\n(Code %d)\n", filename, result);
            }
        }
